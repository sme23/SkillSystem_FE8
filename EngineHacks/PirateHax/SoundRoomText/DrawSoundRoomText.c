...
u16 GetCurrentSoundRoomSongSourceText(){
	struct SoundRoomProc* currentSoundRoomUIProc = ProcFind(gSoundRoomUIProc);
	u8 currentSoundRoomIndex = currentSoundRoomUIProc->curIndex;
...
void DrawSoundRoomSongSource(){
	...
	char* string = GetStringFromIndex(GetCurrentSoundRoomSongSourceText());

	unsigned fullLen = 128;
	
	Text_InsertString(someTextHandleArray, Text_GetStringTextCenteredPos(fullLen, string), 0, string);
